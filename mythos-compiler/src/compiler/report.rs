//! `mythos report`: renders `<run-dir>/state/report.html`, a single
...
fn render_verdict_banner(gate: Option<&GateReport>) -> String {
    match gate {
        ...
        None => "<section class=\"verdict verdict-unknown\">GATE NOT RECORDED - run mythos-skill gate</section>\n"
            .to_string(),
    }
}
...
const FOOTER: &str = "<footer><p>Generated by mythos report - Receipts engine</p></footer>\n";
...
pub fn generate_report(run_dir: &Path) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let packet_text = fs::read_to_string(&packet_path).map_err(|err| {
        format!(
            "failed to read {}: {err} - run `mythos compile --run-dir <dir>` first",
            packet_path.display()
        )
    })?;
...
mod tests {
    use super::*;
    use crate::schema::{CompiledFact, MYTHOS_SCHEMA_VERSION};
