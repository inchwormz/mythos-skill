...
function printHelp() {
  ...
  process.stdout.write(
    `mythos-skill ${pkg.version} — explicit-state recurrent synthesis for AI agent runs\n\n` +
      ...
      `    mythos-skill absorb --run-dir my-run --lane L1 --agent-id a --from agent.md\n` +
      `    mythos-skill conclude --run-dir my-run --synthesis "what happened this pass"\n` +
...
// Spawn the resolved `mythos` binary (a full path from which()/where). Same
// no-shell reasoning as spawnScript.
function spawnMythos(mythosPath, mythosArgs) {
  return spawnSync(mythosPath, mythosArgs, { encoding: "utf8" });
}
...
function cmdAbsorb(args) {
  ...
  if (!runDir || !lane || !agentId || !from) {
    process.stderr.write(
      "Usage: mythos-skill absorb --run-dir <d> --lane <l> --agent-id <a> --from <file> [--no-diff]\n",
    );
    return 2;
  }
  ...
  if (ingestResult.error) {
    process.stderr.write(`mythos-skill absorb: failed to run ingest-subagent.mjs: ${ingestResult.error.message}\n`);
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write(
      `mythos-skill absorb: ingest-subagent.mjs did not print valid JSON to stdout: ${error.message}\n`,
    );
    return 1;
  }

  // Step 2: `mythos diff` unless --no-diff. NON-FATAL — repo_root may not be
  // a git repo, or the binary may not be installed; warn and continue.
  let workReceipt = null;
  if (!noDiff) {
    const mythos = which("mythos");
    if (!mythos) {
      process.stderr.write("mythos-skill absorb: warning: `mythos` binary not found on PATH; skipping diff receipt\n");
    } else {
      const diffResult = spawnMythos(mythos, ["diff", "--run-dir", runDir, "--note", `post-lane ${lane}`]);
      if (diffResult.error) {
        process.stderr.write(`mythos-skill absorb: warning: mythos diff failed to launch (${diffResult.error.message}); continuing without a work receipt\n`);
      } else if (diffResult.status !== 0) {
        process.stderr.write(
          `mythos-skill absorb: warning: mythos diff exited ${diffResult.status} (${firstStderrLine(diffResult, "no stderr")}); continuing without a work receipt\n`,
        );
      } else {
        try {
          workReceipt = JSON.parse(diffResult.stdout).receipt ?? null;
        } catch (error) {
          process.stderr.write("mythos-skill absorb: warning: mythos diff did not print valid JSON; continuing without a work receipt\n");
        }
      }
    }
  }
  ...
  if (compileResult.error) {
    process.stderr.write(`mythos-skill absorb: failed to run driver.mjs: ${compileResult.error.message}\n`);
    return 1;
  }
...
function cmdConclude(args) {
  ...
  if (!runDir || !synthesis) {
    process.stderr.write('Usage: mythos-skill conclude --run-dir <d> --synthesis "<text>" [--skip-report]\n');
    return 2;
  }
  ...
  if (synthesisResult.error) {
    process.stderr.write(`mythos-skill conclude: failed to run driver.mjs: ${synthesisResult.error.message}\n`);
    return 1;
  }
  ...
  if (gateResult.error) {
    process.stderr.write(`mythos-skill conclude: failed to run strict-gate.mjs: ${gateResult.error.message}\n`);
    return 1;
  }
  ...
  if (!skipReport) {
    const mythos = which("mythos");
    if (!mythos) {
      process.stderr.write("mythos-skill conclude: warning: `mythos` binary not found on PATH; skipping report\n");
    } else {
      const reportResult = spawnMythos(mythos, ["report", "--run-dir", runDir]);
      if (reportResult.error) {
        process.stderr.write(`mythos-skill conclude: warning: mythos report failed to launch (${reportResult.error.message})\n`);
      } else if (reportResult.status !== 0) {
        process.stderr.write(
          `mythos-skill conclude: warning: mythos report exited ${reportResult.status} (${firstStderrLine(reportResult, "no stderr")})\n`,
        );
      }
    }
  }
  ...
  const nextResult = spawnMythos(mythosNext, ["next", "--run-dir", runDir]);
  if (nextResult.error) {
    process.stderr.write(`mythos-skill conclude: warning: mythos next failed to launch (${nextResult.error.message})\n`);
  } else {
