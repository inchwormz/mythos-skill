...
> **Receipts** — proof-of-work for AI agent claims. Engine plan: [RECEIPTS-ENGINE-PLAN.md](./RECEIPTS-ENGINE-PLAN.md) · campaign state: [RECEIPTS-LEDGER.md](./RECEIPTS-LEDGER.md). (GitHub repo URL still says mythos-skill until the rename lands.)
...
[![crates.io](https://img.shields.io/crates/v/receipts-core)](https://crates.io/crates/receipts-core) [![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](./LICENSE)

## Why

LLM agent pipelines drift when Prime (the orchestrating model) reads subagent chat directly. Claims slip in without provenance, contradictions get glossed over, and re-running the same objective produces different packets. Receipts enforces a hard boundary: subagents write fenced machine-readable records into a run directory, the compiler hashes and validates them, and Prime only ever sees the compiled packet. On top of that, runtime-minted execution receipts close the gap between a lane *claiming* "tests passed" and the orchestrator having actually watched it happen — the difference between **asserted** evidence (the agent wrote it about itself) and **attested** evidence (the runtime observed it).

## Mental model

Receipts splits into two kinds of artefact — a **runtime** you install once and **skill packages** you install per Prime surface.
...
│  skills/receipts/   │              │  skills/receipts/   │              │                   │
│  SKILL.md         │              │  SKILL.md         │              │                   │
└─────────┬─────────┘              └─────────┬─────────┘              └─────────┬─────────┘
          │                                  │                                  │
          ▼                                  ▼                                  ▼
          Prime calls `receipts` per the skill contract.
```

The runtime is the same for everyone (binary `receipts-core`, CLI `receipts` — renamed at M3 as planned). The skill package is a contract file the host AI surface reads to know *how* to call the runtime: which subagent lanes to spawn, when to absorb their output, when to mint receipts, when to conclude.
...
Prime (Claude Code / Codex / custom)
  │
  ├─ receipts init <run-dir> [--repo-root <path>]
  ...
  │     Convention: put the run dir under <cwd>/.receipts/runs/<name> in the
  ...
  │     receipts absorb --run-dir <d> --lane <l> --agent-id <a> --from <file>
  ...
  │     receipts run --run-dir <d> --label test:<name> -- <command...>
  ...
  │
  ├─ receipts resolve --run-dir <d> --target <id> --reason "<why>" [--cite <source-id>]
  │     Only when a BLOCKING worklist item is genuinely dispositioned.
  │     Records a hash-chained adjudication; recompile to apply.
  │
  └─ receipts conclude --run-dir <d> --synthesis "<source-backed summary>" [--skip-report]
...
- **Derived worklist + hash-chained resolutions** — the compiler derives `candidate_actions` from contradictions, gaps, and blockers and classifies each as `blocking` or advisory. `receipts resolve --run-dir <d> --target <id> --reason "..." [--cite <source-id>]` records an append-only, hash-chained adjudication that clears a blocking item on the next compile.
...
- **Forgiving, zero-burden ingest** — lane briefs are task-only; agents get no format instructions. Fenced `receipts-evidence-jsonl` / `receipts-verifier-jsonl` (legacy `mythos-*` labels accepted) blocks are parsed and repaired liberally when present; unlabeled prose gets its claims harvested sentence-by-sentence (any concrete path or `file.ext:line` citation becomes its own hash-verified record); anything left over becomes a single demoted `unstructured` record. Every repair is logged in the ingest report. Extraction can never manufacture trust — promotion still needs receipts or verifier backing.
- **The strict gate** — `receipts gate --run-dir <d>` (folded into `conclude`) checks input freshness, coverage, subagent traceability, direct-source-ref ratios, machine-specific path leakage, receipt refutations, and unresolved blocking worklist items before a run counts as "done".
...
cd mythos-skill
cargo install --path receipts-compiler   # builds and installs the `receipts-core` binary
npm link                                # links bin/receipts.mjs as the `receipts` command
receipts ready                      # end-to-end self-test — must print "receipts readiness: passed"
```
...
```bash
cargo install receipts-core                      # Rust engine binary (receipts-core) - publish pending
...
**Claude Code** — installs to `~/.claude/skills/receipts/SKILL.md`:
...
**Codex** — installs to `~/.codex/skills/receipts/SKILL.md`:
...
#    --repo-root); convention is to nest it under .receipts/runs/ in the
#    project you're actually working on.
receipts init .receipts/runs/my-run

# 2. After a lane finishes and writes raw/subagents/lane-a.md, absorb it in
#    one motion: ingest -> work receipt -> recompile.
receipts absorb --run-dir .receipts/runs/my-run --lane lane-a --agent-id agent-1 \
  --from .receipts/runs/my-run/raw/subagents/lane-a.md
...
receipts run --run-dir .receipts/runs/my-run --label test:suite -- cargo test

# 4. Clear a blocking worklist item once it's genuinely dispositioned
#    (see the id in `receipts next`'s worklist).
receipts resolve --run-dir .receipts/runs/my-run --target <id> --reason "<why this is adjudicated>"

# 5. Conclude the pass in one motion: synthesis -> recompile -> gate ->
#    report -> the Prime brief. Exit code is the gate's.
receipts conclude --run-dir .receipts/runs/my-run \
  --synthesis "one-paragraph summary with direct citations"
```

`conclude` prints the brief for you; re-read it any time with `receipts next --run-dir .receipts/runs/my-run`.
...
- **Receipts authenticate execution, not the executor.** Any process on the
  box — including a lane with shell access — can invoke `receipts run`
...
- `receipts-compiler/` — Rust crate (`receipts-core`, crates.io publish pending; subcommands `init`, `run`, `diff`, `resolve`, `compile`, `report`, `next`)
- `bin/receipts.mjs` — Node CLI dispatcher: passes `init`/`run`/`diff`/`resolve`/`next` straight through to the `receipts-core` binary, runs `compile`/`ingest`/`gate`/`ready` itself, and provides the composite commands `absorb` and `conclude` that chain the above into one motion per lane / per pass
...
- `driver.mjs` — Node entrypoint invoked by `receipts compile`; creates or recompiles a run and prints the packet
